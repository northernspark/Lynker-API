...
if (empty($passed_type)) {
	...
	if ($network_count == 0) {
		...
		$network_post = mysql_query("INSERT INTO `networks` (`network_id`, `network_key`, `network_type`, `network_user`, `network_status`, `network_token`, `network_username`, `network_page`, `network_data`) VALUES (NULL, '$network_key', '$passed_type', '$user_key', '$passed_status', '', '$passed_username', 'passed_page', '$passed_data');");
	}
	else {
		$network_key = $network_data['network_key'];			
		$network_post = mysql_query("UPDATE `networks` SET `network_status` = '$passed_status', `network_token` = '$passed_token', `network_username` = '$passed_username', `network_page` = '$passed_page', `network_data` = '$passed_data' WHERE `network_key` LIKE '$network_key';");
