...
elseif (empty($passed_password)) {
	...
	$user_query = mysql_query("SELECT * FROM `users` LEFT JOIN `companies` on `users`.`user_company` LIKE `companies`.`company_key` WHERE `user_status` LIKE 'active' AND `user_emails` LIKE '%$passed_email%' LIMIT 0, 1");
	$user_count = mysql_num_rows($user_query);
	if ($user_count == 1) {
		while($row = mysql_fetch_array($user_query)) {	
			if (password_verify($passed_password, $row['user_password'])) {
				...
				$user_name = array('fullname' => $user_name, 'firstname' => $user_names[0], 'lastname' => $user_names[1], 'nickname' => $user_nickname);
